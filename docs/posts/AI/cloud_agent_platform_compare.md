...
| 도구 게이트웨이 | Gateway GA. REST, OpenAPI, Lambda를 MCP 도구로 변환. 외부 MCP 서버와 A2A 에이전트는 변환 없이 그대로 연결(패스스루) | Toolbox. 도구를 한 번 등록해 관리형 MCP 엔드포인트로 노출. 도구 검색과 Skills 프리뷰 | Agent Gateway GA(2026년 6월 18일). 사용자, 에이전트, 도구 사이에서 정책을 적용하는 지점. 별도로 Apigee, MCP Toolbox for Databases |
...
| 레지스트리 | AWS Agent Registry GA(2026년 8월 31일. 버지니아 북부, 오레곤, 아일랜드, 도쿄, 시드니 5개 리전, 서울 미지원). 에이전트, 도구, 스킬, MCP 서버 카탈로그와 AgentCore Runtime, Gateway 자동 탐지 | Agent 365와 Entra Agent ID | Agent Registry GA(2026년 6월 18일). 에이전트, 도구, MCP 서버 카탈로그 |
