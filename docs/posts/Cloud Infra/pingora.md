...
- 형태: 크레이트 묶음 → 요청 처리 로직을 작성해 자기 바이너리를 빌드
...
- Windows: 커뮤니티 기여에만 의존하는 예비 지원 단계
- 아키텍처: x86_64와 aarch64
- 릴리스: 최신 stable 0.8.1(2026-06-04). 이 태그 기준 MSRV는 Rust 1.84이고 6개월 rolling 정책
...
| 커넥션 풀 범위 | upstream keepalive 커넥션이 워커 프로세스별 캐시에 보존 | 커넥터의 풀을 같은 서비스의 스레드끼리 공유. 프록시 서비스가 하나면 프로세스 전체가 풀 하나 |
| 설정 방식 | `nginx.conf`의 지시어(directive) | Rust 코드. 설정 파일은 서버 런타임 항목만 담당 |
| 배포 형태 | 패키지로 설치해 바로 기동 | 크레이트 의존성. 직접 빌드한 바이너리로 배포 |
| 확장 방법 | C 모듈 컴파일 또는 njs 스크립트 | `ProxyHttp` 트레이트의 phase 콜백 구현 |
...
| 항목 | 측정값 |
|---|---|
| 커넥션 재사용률 | 한 대형 고객 기준 87.1% → 99.92%. 오리진으로 나가는 신규 커넥션이 160분의 1로 감소 |
...
## 요청 처리 phase
요청 하나가 지나는 지점마다 콜백이 정의돼 있고, `ProxyHttp` 트레이트에서 필요한 것만 구현하는 구조
...
| phase | 호출 시점 | 용도 |
|---|---|---|
| `early_request_filter()` | 모든 요청의 첫 단계 | 다운스트림 모듈의 동작을 요청별로 세밀하게 제어 |
| `request_filter()` | 요청 헤더를 받은 뒤 | 입력 검증, rate limiting, 컨텍스트 초기화 |
| `request_body_filter()` | 요청 본문을 upstream으로 보낼 준비가 된 뒤 | 요청 본문 가공 |
| `proxy_upstream_filter()` | 캐시에서 응답하지 못한 뒤, upstream 연결 직전 | upstream까지 갈지 여기서 끊을지 판단 |
...
| 연결 수립 후 프록시 중 오류 | `error_while_proxy()` | 기본 구현이 재사용 커넥션에서 난 오류인지와 재전송 버퍼가 잘리지 않았는지로 재시도 여부를 결정. 공식 문서는 재사용 커넥션 + 멱등(idempotent) 메서드일 때 재시도할 수 있다고 설명 |
...
- 재시도 횟수 상한은 설정 키 `max_retries`이고 기본 16
...
- `default` feature가 빈 목록 → 필요한 기능을 feature로 켜서 씀
...
- 같은 워크스페이스에 `pingora-http`, `pingora-error`, `pingora-timeout`, `pingora-limits`, `pingora-ketama`, `pingora-memory-cache`, `pingora-pool`, `pingora-lru`, `pingora-runtime`, `pingora-header-serde`, TinyUfo가 함께 있음
...
sequenceDiagram
    ...
    Old->>New: 리스닝 소켓 fd 전달<br>(unix 도메인 소켓 + SCM_RIGHTS)
    New->>Client: 신규 커넥션 accept 시작
    Note over Old,New: 5초 동안 두 프로세스가 함께 accept
    Old->>Client: 진행 중 요청만 마무리 후 종료
```

- 보장 1: 서버 엔드포인트로 접속하는 요청이 connection refused를 보지 않음
- 보장 2: grace period 안에 끝나는 요청은 도중에 끊기지 않음
- 순서: 두 프로세스가 같은 `upgrade_sock` 경로를 보게 맞춤 → 신 프로세스를 `--upgrade`로 기동 → 구 프로세스에 SIGQUIT
- 바인드 주체는 신 프로세스 → 신 프로세스가 `upgrade_sock`을 bind하고 구 프로세스가 connect해서 fd를 보냄
- 전달 수단은 unix 도메인 소켓의 SCM_RIGHTS이고 한 번에 최대 32개 fd
- 양쪽 다 기본 5회, 1초 간격으로 재시도 → 신 프로세스 기동과 SIGQUIT 사이에 5초쯤의 여유가 있음
- fd를 넘긴 뒤에도 구 프로세스가 5초 동안 리스너를 유지 → 그 구간에는 구·신 프로세스가 같은 소켓에서 함께 accept
- 이후 grace period 동안 진행 중 요청만 마무리하고 종료
- 비Linux 빌드에서는 fd 이관 함수가 스텁으로 컴파일됨 → 신 프로세스 쪽은 지원되지 않는다는 오류 로그를 남기고 실패하고, 구 프로세스 쪽 스텁은 로그 없이 성공한 것처럼 반환
...
| `graceful_shutdown_timeout_seconds` | graceful shutdown 마무리 대기 시간. 미설정 시 5초 |
...
## 보안 이력
지금까지 공개된 CVE는 5건. 4건은 HTTP 메시지 경계 해석과 캐시 키 범위 문제이고 1건은 HTTP/2 스트림 리셋 반복에 따른 메모리 소모 문제
...
- CVE-2026-2833/2835/2836 세 건은 2026-03-04에 함께 공개됐고 모두 0.8.0에서 수정 → 둘은 메시지 경계 해석, 하나는 캐시 키 범위 문제
- CVE-2025-4366은 Cloudflare CDN 무료 요금제 이용자와 pingora-proxy·pingora-cache의 캐시 기능 사용자가 노출 대상
...
- MSRV가 6개월 rolling 정책이라 툴체인 갱신 주기를 같이 잡아야 함
