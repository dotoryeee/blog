...
- 무중단 재시작은 양쪽 다 가능. 차이는 새 프로세스를 누가 띄우고 리스닝 소켓을 어떻게 넘기느냐 → nginx는 돌고 있는 마스터가 USR2를 받아 새 바이너리를 자식 프로세스로 직접 띄우고 그 자식이 리스닝 소켓을 상속받음, Pingora는 운영자가 새 프로세스를 따로 띄우고 구 프로세스가 SIGQUIT을 받으면 unix 도메인 소켓으로 리스닝 소켓 fd를 건네줌
