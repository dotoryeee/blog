...
2026년 9월 3일 Public Preview로 공개된 Amazon Linux의 다음 메이저 버전. AL2023 baseline 위에 커널 7.1, SELinux Enforcing 기본, OpenSSL 3.5와 AWS-LC 병행, DNF5, RPM 6.0을 올린 구성

| 항목 | 내용 |
|---|---|
| 상태 | Public Preview. 평가와 테스트 용도이며 프로덕션 워크로드에는 권장하지 않음. AWS Support 계약이 프리뷰를 다루지 않으므로 지원이 필요한 프로덕션은 GA까지 AL2023 유지 |
| 프리뷰 기간 | 2027년 초까지. 릴리스 노트의 프리뷰 패키지 지원 종료일은 2027년 3월 31일. 확인 가능한 공식 문서에 GA 날짜 표기 없음. 2027년 1월 이후 제기된 이슈는 GA 전에 해결되지 않을 수 있고, GA 시점에 프리뷰 AMI는 deprecated 처리되어 패치가 중단됨 |
| 프리뷰 단계 | Phase 1(공개 시점): 코어 패키지, 커널 7.1, SELinux Enforcing, DNF5, 최신 툴체인과 런타임, OpenSSL과 병행하는 AWS-LC, Trainium과 Inferentia용 Neuron 드라이버, GNOME Desktop. Phase 2(2026년 11월): SPAL 패키지, DCV 지원, EKS-optimized AMI 레시피 |
| 프리뷰 제약 | 인스턴스가 기본으로 어떤 업데이트도 자동 수신하지 않음(중요 보안 업데이트 포함). 프리뷰 아티팩트의 open CVE 패치는 보장되지 않음. SSM Patch Manager 패치 작업 실패. NVIDIA 드라이버, 온프레미스용 VM 이미지(Hyper-V, KVM, VMware), FIPS 검증 커널과 암호 모듈은 초기 프리뷰에 없음 |
| GA 이후 지원 | 제품 페이지 FAQ 기준 표준 지원 5년. 사용자 가이드 release cadence 문서는 2032년까지 지원으로 표기하며 두 표기가 maintenance 기간을 어떻게 나누는지는 문서에 없음. 표준 지원 중에는 3개월마다 마이너 릴리스, 이후 maintenance 단계에서는 보안 업데이트와 치명적 버그 수정만 제공. 다음 메이저 버전이 나오면 maintenance 진입 |
| 요금 | 라이선스 비용 없음. 프리뷰 중에도 EC2 인스턴스 요금만 발생 |
| 아키텍처 | x86-64(x86-64-v3 이상)와 aarch64(Graviton2 이상). 32비트 x86과 32비트 ARM 바이너리 없음 |
| 리전 | 모든 상용 AWS 리전에 Preview AMI 제공. AWS Management Console에서 바로 기동 가능 |
| 업스트림 | Fedora 44와 45에 부분 기반. Fedora derivative나 remix는 아니며 릴리스와 지원 주기는 독립. 커널은 kernel.org 릴리스에서 Fedora와 무관하게 선택 |
| 버전 표기 | 2027.0.20260903처럼 메이저, 마이너, 날짜 순. 표준 지원 중 3개월마다 마이너가 올라가고 같은 마이너 안에서는 날짜로 AMI 재발행을 구분. 버전 확인용 CPE 문자열은 /etc/system-release-cpe에 있고 Amazon Linux 전용 식별 파일은 /etc/amazon-linux-release와 /etc/amazon-linux-release-cpe. 새 코드는 /etc/os-release 사용 권장 |
| 피드백 채널 | GitHub amazonlinux/amazon-linux-2027 issues에 "AL2027 Public Preview" 태그로 등록. AWS re:Post, 담당 AWS 대표도 가능. 보안 이슈는 issue 대신 보안팀 연락 |

프리뷰 기간에는 패키지별 지원 기간이 프리뷰 종료 시점까지만 표시됨. 실제 지원 일정은 정식 릴리스와 함께 공개 예정

첫 프리뷰 릴리스 2027.0.20260903의 패키지 규모. 릴리스 노트 요약표 기준

| 비교 | 소스 RPM | 바이너리 RPM |
|---|---|---|
| AL2023.12 대비 새로 추가 | 178 | 614 |
| AL2023.12에서 업그레이드 | 2408 | 8078 |
| AL2023.12에 있고 AL2027에 없음 | 307 | 8049 |
| AL2027 전체 | 2548 | 11633 |
| AL2023.12 전체 | 2726 | 18756 |
