...
| 지표 | 집계 기준 | 설명 | 주 용도 |
|---|---|---|---|
| Unblended Cost | 현금주의 결제 시점 | 계정 단위 실제 청구 단가 기준. 인보이스 금액과 일치 | 회계 대조, 실지출 추적 |
| Blended Cost | 조직 평균 단가 | Organizations 통합 결제 내 전체 사용량을 합산한 평균 단가 기준. 공식 폐기는 아니지만 왜곡 가능성 때문에 실무에서는 기피 | 레거시 리포트 |
| Amortized Cost | 발생주의 사용 기간 | 선급금(Upfront)·약정(SP/RI) 비용을 계약 기간에 걸쳐 일할/시간 단위로 균등 분배 | 트렌드 분석, 부서별 배분 |
| Net Amortized Cost | 발생주의 + 협상 할인 | Amortized Cost에 EDP/PPA 등 협상 할인까지 반영한 순상각 비용 (CUR의 NetEffectiveCost 계열 컬럼) | 실질 원가 산출 |
| List Cost (On-Demand Cost) | 정가 | 약정·할인이 전혀 적용되지 않은 공시 단가 기준 | 절감액(Savings) 계산의 기준선 |

### Unblended vs Amortized 예시
...
|---|---|---|---|
| Cost Explorer | 콘솔 대시보드 (집계·시각화) | 서비스·계정 단위 요약 | Unblended/Amortized 등 지표를 옵션으로 선택 |
| Legacy CUR (1.0) | S3로 내보내는 원천 청구 원장 | 리소스 ID·시간 단위 | 사용 서비스/태그에 따라 월별 컬럼 수 변동 |
...
|---|---|---|---|---|---|
| Reserved Instance (RI) | 리소스 (인스턴스 규격·수량) | EC2, RDS·ElastiCache·Redshift·OpenSearch 등 | ~72% (3년) | Zonal RI만 | 정적 1:1 매핑. Standard/Convertible × Regional/Zonal 조합 |
| Savings Plans (SP) | 금액 (시간당 지출 $/hr) | EC2·Fargate·Lambda, DB 계열, SageMaker AI | ~72% (3년) | 없음 | 매시간 자동 적용. Compute/Database/EC2 Instance/SageMaker AI 4종 |
