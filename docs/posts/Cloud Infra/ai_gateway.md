...
|Portkey|셀프호스팅·관리형 SaaS. MIT 오픈소스(2026년 거버넌스·관측 기능까지 오픈소스화).|지원. 가중치 로드밸런싱·조건부 라우팅·폴백·자동 재시도.|Simple은 전 플랜. Semantic은 벡터DB 필요, 일부 Enterprise.|지원. 50+ 가드레일 플러그인.|지원. 요청·비용·지연 대시보드.|1,600+ LLM. 가드레일 통합이 강점. Docker·k8s 셀프호스팅.|
...
- during-call: LLM 호출과 병렬로 입력만 검사해 지연을 줄이는 용도로, 위반 감지 시 예외를 던져 요청 거부 가능
