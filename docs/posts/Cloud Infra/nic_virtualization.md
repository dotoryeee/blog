...
에뮬레이션 NIC의 가상화 I/O 오버헤드에서 출발해 virtio, vhost, SR-IOV, DPDK, DPU까지 진화 순서로 정리했다. 방식별 데이터 경로와 트레이드오프를 비교한다.
...
- 게스트에 virtio 프론트엔드 드라이버, 호스트에 백엔드 드라이버를 두고 둘이 공유 메모리로 통신
- 통신 통로는 virtqueue → 그 실체는 링 버퍼인 vring
- 프론트엔드가 요청을 vring에 넣고 백엔드에 알림(notify) → 실제 하드웨어 레지스터 접근을 대체해 exit 횟수를 줄임
...
- 게스트는 그대로 virtio 프론트엔드만 봄 → 백엔드만 교체해 성능을 끌어올리는 구조
