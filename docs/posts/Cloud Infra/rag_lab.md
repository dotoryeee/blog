...
Qdrant와 로컬 임베딩 모델로 RAG 파이프라인을 직접 세워봤다. 청킹, 색인, 하이브리드 검색, 리랭킹을 단계별로 실측해 확인한 기록이다.
...
- BM25를 섞은 하이브리드(RRF)와 dense 단독 검색을 같은 질의셋으로 대조한다
- dense가 올린 후보 20개를 리랭커로 다시 매겨 순위가 얼마나 바뀌는지, 못 바꾸는 경우는 언제인지 확인한다
- 같은 질문을 MoE와 Dense 두 모델에 각각 RAG 없이 한 번, 검색 결과를 붙여 한 번 넣어 답이 어디서 갈리는지 본다

검색 품질을 떨어뜨리는 함정과 단계별 선택지는 [RAG 정리](rag.md)에서 표로 다뤘다. 이 글은 그 표에 적힌 주장 중 몇 개를 실제 숫자로 확인하고, 맞지 않은 것도 그대로 적는다.
...
Dense는 MoE를 내린 뒤 같은 방식으로 8081 포트에 올린다. 검색 결과와 프롬프트는 MoE 때 쓴 것을 파일로 저장해 두고 그대로 재사용한다.
...
속도는 이 조건에서도 갈린다. 검색 결과를 붙인 조건에서는 프롬프트 처리까지 포함한 wall-clock으로 MoE가 5.4배 빠르고, decode 속도만 보면 MoE 45~49 t/s 대 Dense 12.4 t/s로 3.8배 정도다.

!!! notice
    💡 이 표의 decode 속도는 llama-server가 응답에 실어 주는 timings 값이라 llama-bench 수치와 계측 방식이 다르다

llama-bench는 같은 조건을 여러 번 돌려 평균을 내지만 여기 값은 58~64토큰짜리 단발 생성 하나에서 나왔다. MoE가 45.2와 48.9로 흩어진 것이 그 편차 폭이다. 방법론은 순수 decode 벤치마크에서 잰 4.07배와 다르지만 방향과 배율은 비슷하게 재확인됐다.
