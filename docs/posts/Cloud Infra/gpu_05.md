...
- 토폴로지·메시지 크기로 알고리즘을 자동 선택 → 큰 텐서는 Ring, 작은 메시지는 Tree가 기본 구도
- NVSwitch가 있는 Hopper 이상 노드에서는 NVLS(NVLink SHARP)가 기본 후보 → 스위치가 reduce 연산을 대신 수행해 Ring처럼 GPU끼리 조각을 주고받지 않음
...
순수 DP는 전 GPU가 모델 상태를 통째로 복제하므로 큰 모델에서 메모리가 먼저 한계에 부딪힘.
