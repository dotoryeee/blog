...
- 과금: 기록한 건수만큼 청구 → 관리 이벤트가 계정당 한 벌씩 공짜로 받는 무료분이 네트워크 활동 이벤트에는 없음
...
## userIdentity.arn 필터
호출한 IAM 자격 증명에 따라 기록 여부가 갈림. `userIdentity.arn`은 데이터 이벤트와 관리 이벤트에서도 쓰이는 필드이고, 네트워크 활동 이벤트 지원은 2026년 7월에 추가됨
