...
| Geneve | 약 50B(+옵션 가변) | 1500 − 50 = 1450 (옵션 가산 시 감소) | 자체 암호화 없음, 언더레이 암호화 | IPsec ~36B 등 합산 | 예: 1500 − (64 + 36) = 1400 | 옵션(TLV)로 가변 헤더 → MTU 마진 넉넉히 확보 권장 |
...
- Geneve(옵션 14B 가정, 총 64B): 1500 − 64 = 1436  
- VXLAN + WireGuard: 1500 − (50 + 60) = 1390  
- IPIP + IPsec(ESP ~36B): 1500 − (20 + 36) ≈ 1444

### 참고
- MTU 계산 원칙: 유효 MTU = 언더레이 MTU − (캡슐화/암호화 헤더 오버헤드 합). 일반적으로 IPv4 기준 대략 IP-in-IP 20B, VXLAN 50B, Geneve 50B(옵션에 따라 가산)로 본다
...
- Geneve 기본 오버헤드는 VXLAN과 같은 ~50B, 옵션(TLV)에 따라 가산(안정적 MTU를 위해 보수적으로 산정)
