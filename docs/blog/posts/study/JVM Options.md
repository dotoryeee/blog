...
이 옵션은 64비트 JVM에서 객체 참조(포인터)를 압축하여 메모리 사용량을 줄입니다. 객체 포인터가 32비트 정수로 저장됩니다.
...
이 옵션은 새로 생성된 객체가 Old 영역으로 승격되기 전까지 Survivor 영역에서 살아남아야 하는 마이너 GC 횟수(age)를 설정하는 데 사용됩니다. 세대별 GC에서 공통으로 사용하며 기본값과 최대값은 15입니다.

## CMS GC에서 G1 GC로 전환할 때 주의할 점

- Xmn, -XX:NewRatio를 사용하지 않습니다.<br>
G1 GC는 전체 힙을 일정한 크기의 region으로 나누고 각 region을 Young(Eden/Survivor)이나 Old로 동적으로 할당하기 때문에, Young 영역과 Old 영역이 region 단위로 항상 공존합니다. G1 GC는 목표 정지 시간에 맞춰 Young 영역 크기를 자동으로 조절하므로, -Xmn, -XX:NewRatio와 같은 Young 영역 크기 설정 옵션을 사용하면 이 동작을 방해하기 때문에 사용하지 않습니다.
...
G1 GC의 주요 목표 중 하나는 GC 정지 시간(pause time)을 최소화하는 것입니다. 이를 위해 -XX:MaxGCPauseMillis 옵션으로 목표 최대 정지 시간을 설정할 수 있으며 기본값은 200ms입니다.
...
- 장점<br>
    - 짧은 정지 시간: ZGC는 큰 힙 크기에서도 정지 시간을 10ms 미만으로 제어하는 것을 목표로 하며, 최신 버전에서는 1ms 미만까지 줄어듭니다.
    - 확장성 향상: ZGC는 대규모 시스템에서 수천 개의 CPU와 수백 GB 힙 크기를 지원합니다.
    - 정지 시간 우선 설계: ZGC는 메모리 압축을 포함한 대부분의 작업을 애플리케이션과 동시에 수행하며, 처리량보다 정지 시간을 줄이는 데 최적화되어 있습니다. 다만 이 때문에 처리량 자체는 G1이나 Parallel GC보다 다소 낮을 수 있습니다.
