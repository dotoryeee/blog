...
- Nginx
    ...
    - 하나의 연결은 하나의 worker process가 처리하며, 로드밸런싱은 여러 요청을 upstream 서버들로 분산하는 개념이다
...
<!-- more -->
- httpd Event MPM (httpd 2.4부터 추가된 모드 / 하나의 연결은 하나의 worker thread가 처리하되, keep-alive 연결은 별도 listener thread로 넘겨 worker thread 점유를 줄임)
    - 장점
...
- httpd Prefork MPM (스레드 없이 요청과 프로세스의 1:1 매핑)
    - 장점
...
open file limit은 Prefork MPM에서 발생하는 경우가 많습니다. Prefork MPM은 스레드를 사용하지 않고 하나의 프로세스가 하나의 요청을 처리합니다. 이 때문에 많은 요청이 동시에 발생하면 그만큼 많은 프로세스가 생성되어 open file limit을 초과할 가능성이 있습니다. Event MPM에서는 하나의 프로세스 내에서 여러 개의 요청을 처리하기 때문에 Prefork MPM보다 open file limit의 문제가 적습니다.
...
(httpd event MPM 모드 + 멀티프로세스 + 멀티스레드)와 Nginx는 모두 이벤트 기반 아키텍처를 사용하고, I/O 멀티플렉싱을 통해 블로킹을 최소화하여 높은 성능을 보입니다. 그러나 구조적으로는 다릅니다.

httpd event MPM 모드 + 멀티프로세스 + 멀티스레드: 이 방식은 멀티 프로세스를 사용하며 각 프로세스는 ThreadsPerChild 값만큼의 worker thread를 두고, 별도의 listener thread가 keep-alive 연결을 관리합니다. 따라서 멀티프로세스·멀티스레드로 인한 컨텍스트 스위칭 오버헤드가 있을 수 있지만, 이벤트 기반으로 비동기 I/O를 처리하므로 자원을 효율적으로 사용합니다.
...
따라서, httpd event MPM 모드 + 멀티프로세스 + 멀티스레드 방식은 안정적이지만 성능은 Nginx에 비해 떨어질 수 있습니다. 반면에, Nginx는 IPC 오버헤드가 없기 때문에 처리 속도가 더 빠를 수 있습니다.
