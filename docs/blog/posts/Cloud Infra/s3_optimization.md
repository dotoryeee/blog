...
- Express One Zone은 제공 리전이 제한됨 → ap-northeast-2(서울) 미제공(2026-07 기준), 버지니아·도쿄·프랑크푸르트 등 8개 리전에서만 생성 가능
- Standard-IA·One Zone-IA·Glacier Instant Retrieval은 128KB 미만 객체도 128KB로 과금
- Glacier Flexible Retrieval·Deep Archive는 객체당 40KB 메타데이터 추가 과금(32KB는 Glacier 요율, 8KB는 Standard 요율)
...
graph TD
    ...
    GIR["Glacier Instant Retrieval<br>저장 $0.005/GB·월<br>최소 보관 90일, 검색 $0.03/GB"]
    FR["Glacier Flexible Retrieval<br>저장 $0.0045/GB·월, 최소 보관 90일<br>복원 Standard $0.011/GB, Expedited $0.033/GB, Bulk 무료"]
    GDA["Glacier Deep Archive<br>저장 $0.002/GB·월, 최소 보관 180일<br>복원 Standard $0.022/GB, Bulk $0.005/GB"]
    DEL["만료·삭제<br>최소 보관 충족 후면 추가 과금 없음<br>미충족 이탈은 잔여 기간 과금"]
    S -->|"즉시 전환 가능(2026-07부터)<br>전환 $0.01/1,000건"| IA
    S -->|"즉시 전환 가능(2026-07부터)<br>전환 $0.01/1,000건"| ZIA
    S -.->|"즉시 전환 가능(원래부터)<br>전환 $0.02/1,000건"| GIR
    S -.->|"즉시 전환 가능(원래부터)<br>전환 $0.03258/1,000건"| FR
    S -.->|"즉시 전환 가능(원래부터)<br>전환 $0.06/1,000건"| GDA
    IA -->|"최소 보관 30일 충족 후<br>전환 $0.02/1,000건"| GIR
    GIR -->|"최소 보관 90일 충족 후<br>전환 $0.03258/1,000건"| FR
    GIR -.->|"최소 보관 90일 충족 후<br>전환 $0.06/1,000건"| GDA
    FR -->|"최소 보관 90일 충족 후<br>전환 $0.06/1,000건"| GDA
    GDA --> DEL
```
...
- 작은 객체 다수를 IA 계열·Glacier 계열로 내리면 손해 → 128KB 미만도 128KB로 과금되고 전환 요청비까지 붙음
- 최소 보관 기간 전 삭제·재전환 시 조기 삭제 요금 → Standard-IA에서 30일 전에 지우면 남은 기간만큼 과금
- 조기 삭제는 결국 최소 보관 기간 전체를 낸 셈이 됨 → Glacier Deep Archive 전환 20일 뒤 지워도 잔여 160일분이 과금되어 총 180일치 부담
- 최소 보관을 못 채우는 중간 기착은 단일 규칙에선 지정 자체가 불가 → Glacier Instant Retrieval 진입 90일 이전 시점의 Glacier Deep Archive 전환은 규칙 생성이 거부됨
- 규칙을 둘로 쪼개면 조기 전환은 가능하나 잔여 기간 과금 → Glacier Instant Retrieval에 30일만 머물다 넘겨도 90일치 부담, 짧게 머물 바엔 Glacier Deep Archive 직행이 저렴
...
- Express One Zone은 단일 AZ에 데이터를 두어 왕복 지연을 줄인 최상위 성능 위치 → 디렉터리 버킷 기본 한도는 초당 20만 읽기·10만 쓰기, 지원 요청 증설로 초당 200만 읽기·20만 쓰기까지 확장 가능
...
| 검색 비용 | IA 계열·Glacier 계열은 GET·복원 시 GB당 요금, 자주 꺼내면 Standard보다 비쌈 | 접근 빈도 실측 후 클래스 결정 |
...
- ETag는 객체의 버전 식별자 → 단일 PUT 객체는 데이터의 MD5와 같지만 멀티파트·KMS 암호화 객체는 MD5와 다름, 무결성 검증은 체크섬 기능(SHA-256 등)으로
...
- IA 전환 30일 제한 폐지(2026-07): Standard-IA·One Zone-IA로 생성 즉시 라이프사이클 전환 가능 → 30일 대기를 전제로 한 기존 설계 관행이 무의미해짐
...
| 한 자릿수 ms가 필요한 핫데이터 | Express One Zone | 단일 AZ 근접 저장으로 최저 지연(서울 리전 미제공) |
