...
```mermaid
graph LR
    subgraph W["사용 사례 1: 장기 아카이브 폭포수"]
        S["Standard<br>(0일)"] -->|"전환 요청비(1,000건당)"| IA["Standard-IA (30일+)<br>단가↓ GET시 GB당 검색비"]
        IA -->|"전환 요청비"| GIR["Glacier IR (90일+)<br>단가↓↓ 검색비↑"]
        GIR -->|"전환 요청비"| GDA["Deep Archive (180일+)<br>최저 단가 + 객체당 40KB 메타 가산"]
        GDA -->|"만료(360일+)"| EXP["삭제<br>최소 보관 충족 → 추가 과금 없음"]
    end
```

```mermaid
graph LR
    subgraph B["사용 사례 2: 백업 즉시 IA 전환(2026-07부터)"]
        S2["Standard<br>(0일)"] -->|"생성 즉시 전환<br>전환 요청비"| IA2["Standard-IA<br>충족 시점부터 IA 단가"]
        IA2 -->|"30일 전 삭제·재전환"| EARLY["잔여 기간 과금<br>(최소 보관 30일은 유지)"]
        IA2 -->|"30일 경과 후 삭제"| OK2["추가 과금 없음"]
    end
```
