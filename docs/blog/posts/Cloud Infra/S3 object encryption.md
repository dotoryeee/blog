...
2. S3 암호화 방식은 총 4가지가 존재한다
    ...
    - DSSE-KMS (Dual-layer Server-Side Encryption with AWS KMS): AWS KMS 키를 사용해 두 겹의 AES-256 암호화를 적용한다. 규제 준수 요건이 높은 환경에서 사용된다.
3. 암호화 방식에 따라 오브젝트 업로드 명령 옵션이 상이하다
    ...
    - SSE-C<br>
    ```bash
    aws s3 cp <object> s3://<bucket>/<key> --sse-c AES256 --sse-c-key <encryption-key>
    ```
    - SSE-KMS<br>
    ```bash
...
4. SSE-KMS를 이용해 오브젝트 up/download 하는 python code 예시
    ...
    # S3 객체 다운로드 (SSE-KMS 객체는 KMS 복호화 권한이 있으면 get_object 시 자동 복호화됨)
