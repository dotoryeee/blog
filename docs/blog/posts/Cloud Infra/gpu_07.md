...
- GQA(Grouped Query Attention)를 쓰는 모델은 KV 헤드 수가 어텐션 헤드보다 적음 → Llama 3.1 8B는 헤드 32개 중 KV 헤드 8개라 KV가 1/4로 줄어듦
...
- 8B 모델도 컨텍스트를 최대 131,072토큰까지 키우면 시퀀스 하나의 KV가 16GiB → 긴 컨텍스트는 가중치보다 KV가 VRAM을 지배
