...
## 서빙 엔진 비교
엔진마다 KV 관리·컴파일 방식·하드웨어 지원이 달라 워크로드에 맞춰 고름

| 엔진 | 개발 | 강점 | 적합 상황 |
|------|------|------|-----------|
| vLLM | 오픈소스(PagedAttention 원조) | 범용 기본값, 폭넓은 하드웨어(CUDA·ROCm·TPU·Gaudi·CPU) | 프로덕션 기본 서빙 |
| TensorRT-LLM | NVIDIA | 모델을 TensorRT 엔진으로 컴파일, NVIDIA에서 최고 처리량 | NVIDIA 전용 극한 최적화 |
...
- TensorRT-LLM은 모델별 엔진 빌드·튜닝이 필요 → NVIDIA 하드웨어에 고정해 최고 성능을 뽑을 때
