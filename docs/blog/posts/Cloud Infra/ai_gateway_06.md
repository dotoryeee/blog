...
|과금 남용|가상 키 유출로 토큰을 과소비|가상 키·rate limit·예산 한도 (2편에서 다룸)|
...
2편의 litellm_config.yaml에 guardrails 블록을 추가하는 형태. 설정 형태는 다음과 같음(공식 문서 기준)
...
- 다음 편에서는 게이트웨이 관측성(로그·메트릭·트레이싱)을 정리할 예정
