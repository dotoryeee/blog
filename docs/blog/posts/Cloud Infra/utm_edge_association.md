...
- 수평 확장: 어플라이언스를 target group으로 묶어 분산 → 부하 증가 시 Auto Scaling으로 대수만 추가 (ENI 방식은 인스턴스 타입 업그레이드뿐)
- Flow 고정(sticky): 5-tuple 해시로 같은 flow는 항상 같은 어플라이언스에 전달 → 장비 간 세션 동기화(클러스터링) 없이 stateful 검사 유지
- 장애 처리 내장: 헬스체크로 비정상 장비 자동 제외 후 재해싱 → ENI 방식의 수동 라우트 교체(수십 초~수 분 단절) 불필요
- 무중단 증설: GENEVE가 원본 패킷을 그대로 전달하므로 NAT·라우팅 변경 없이 target group 등록/해제만으로 장비 추가·제거 가능
- 멀티 VPC 공유: GWLBe가 PrivateLink 기반이라 여러 VPC·계정이 하나의 검사 fleet을 공유 → 검사 계층을 한곳에서 중앙 확장
