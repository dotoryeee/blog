...
Mac Studio M1 Max에서 Draw Things CLI로 영상 생성 모델 LTX-2.3을 돌려봤다. 스텝, 해상도, 길이별 소요 시간과 메모리 한계를 실측한 기록이다.
...
Apple Silicon은 통합 메모리를 VRAM으로 사용할 수 있어 64GB 장비가 22B 영상 모델을 올릴 수 있음. 문제는 용량이 아니라 프레임워크 지원임.
...
- 모델·VAE·텍스트 인코더·노드를 직접 맞춰야 함 → 조합이 어긋나면 샘플러 단계에서 실패
...
Gemma는 프롬프트를 임베딩으로 바꾸는 인코더이므로 생략 불가. 컨텍스트가 약 1,000토큰에서 잘리고 초과분은 버려짐 → 긴 프롬프트는 낭비.
...
스텝(Step)이란 추론할 때 노이즈에서 영상을 깎아내는 반복 횟수. 체크포인트가 디스크에 놓인 가중치 파일이라면 스텝은 그 파일을 읽어 실제로 그림을 만드는 과정의 반복 횟수이므로 둘은 별개임. 스텝을 늘리면 같은 체크포인트로도 디테일이 올라가고 대신 시간이 늘어남.
...
30편을 연속 생성하는 동안 스왑 증가량을 매 회차 기록함.
...
부하가 1.4배 적은 쪽이 스왑을 18GB 밀어냈음. macOS가 스왑 파일을 2GB에서 3GB로 확장할 정도임.

- 총 부하가 아니라 프레임 수 자체가 병목 → 어텐션 메모리가 프레임 축에서 먼저 터짐
- FHD로 4초를 뽑는 편이 480p로 15초를 뽑는 것보다 안전
- 300프레임 초과가 유일한 위험 구간

30편 전 회차의 스왑 증가는 0MB였음. 위험 구간은 이 배치 밖의 377프레임 시도에서만 나타남.
