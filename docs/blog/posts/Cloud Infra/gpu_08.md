...
full fine-tuning에 VRAM이 왜 그렇게 필요한지부터 PEFT, LoRA, QLoRA의 원리까지 정리했다. 학습 파이프라인과 함께 파인튜닝과 RAG 중 무엇을 골라야 하는지도 비교한다.
...
## 결론
- full fine-tuning은 가중치만이 아니라 그래디언트와 옵티마이저 상태가 파라미터마다 붙어 학습 메모리가 약 18B/param → PEFT가 이 배수를 무너뜨림
