...
|단계|설명|비고|
|--|------|----|
|1|UEFI 펌웨어가 ESP(EFI 시스템 파티션)의 GRUB EFI 애플리케이션(grubx64.efi)을 직접 실행|EFI 파티션 사용|
|2|grubx64.efi가 grub.cfg를 읽어 부팅 메뉴 구성|grub.cfg 설정 파일 참조|
|3|GRUB가 커널 이미지를 메모리에 로드|리눅스 커널(vmlinuz)과 초기 램디스크(initrd)|
