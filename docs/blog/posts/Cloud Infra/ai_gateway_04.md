...
- 라우팅·폴백(Routing·Fallback) 전략을 docker compose로 띄운 LiteLLM + Ollama 게이트웨이 랩에서 직접 실측해봅니다
- 같은 별칭에 백엔드 두 개를 묶어 로드밸런싱하고 weight와 routing_strategy로 분산을 조정합니다
- 재시도·타임아웃과 2단 폴백 체인을 응답 헤더와 컨테이너 로그로 검증합니다

!!! tip
    💡 로컬 ollama·litellm 스택을 그대로 쓰므로 추가 비용이 없습니다

## 로드밸런싱

---

랩의 기본 구성은 llama-small, qwen-small처럼 별칭 하나에 백엔드 하나를 붙인 형태입니다. 로드밸런싱은 같은 별칭에 여러 배포(Deployment)를 등록해 하나의 모델 그룹(Model Group)으로 묶는 방식입니다.

1. 기존 model_list에 chat-small 그룹을 추가합니다. model_name이 같은 항목 두 개가 한 그룹이 됩니다
...
폴백은 llama-small이 죽으면 qwen-small로 넘기는 1단이 기본이고, 후보를 순서대로 이어 체인으로 늘릴 수 있습니다.
