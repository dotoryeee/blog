...
증상이 "커넥션이 막힌 것"처럼 보여서 커넥션 풀 고갈을 처음 의심했다. 커넥션이 막힌다면 보통 DB가 바쁠 때다. 그런데 지표를 다시 보니 DB CPU가 12% 남짓이었다. 자원이 포화 상태가 아닌데도 커넥션이 계속 점유된다면, 전체 부하보다는 특정 쿼리 하나가 오래 잡고 있을 가능성이 크다.
