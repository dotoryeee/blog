...
운영 서비스가 "느리다"는 제보와 "배치가 계속 실패한다"는 제보가 동시에 들어왔다. 며칠간 아무도 몰랐고, 두 증상은 서로 다른 문제처럼 보였지만 뿌리는 하나였다. 커넥션 풀부터 의심했다가 방향을 잘못 잡았고, 결국 "DB CPU가 낮다"는 한 줄의 모순이 원인을 찾는 단서가 되었다.
...
| 슬로우 쿼리 로그 | 한 쿼리가 46초+ (큰 대상은 150초+) | 원인 후보 확정 |
...
슬로우 쿼리 로그가 후보를 좁혀 줬다면, 원인을 확정한 건 RDS Performance Insights였다.
...
Top SQL을 부하순으로 정렬하니 1위가 곧장 드러났다. 한 쿼리가 평균 7,509ms/call로 부하를 독식하고 있었다(나머지는 3ms 안팎). 슬로우 쿼리 로그의 "최대 46초"는 데이터가 많은 큰 그룹에서 나온 최댓값이었고, 평균만 7.5초여도 15분마다 강제로 도니 랭킹 최상단을 놓지 않았다. 흥미롭게도 같은 목록엔 조치로 돌린 `CREATE INDEX CONCURRENTLY`도 함께 잡혀 있었다. 원인과 처방이 한 화면에 나란히 놓인 셈이다.
