...
RAG를 처음 공부하면서 기업이 실제 서비스에 쓰는 방식만 골라 정리했다. 기본 구조부터 하이브리드 검색, 리랭킹, 가드레일, 그래프, 에이전틱까지 전부 공개된 실사용 사례로 확인한 기록이다.

<!-- more -->

## RAG의 기본 구조

RAG 자료를 열면 청킹이니 임베딩이니 하는 말부터 쏟아진다. 처음엔 용어에 막혀 진도가 안 나갔는데 뜯어보니 흐름 자체는 단순했다.

문서가 길면 모델에 한 번에 못 넣는다. 그래서 조각으로 자른다(청킹). 자른 조각은 뜻이 비슷할수록 가까운 값이 나오는 숫자 목록으로 바꿔서(임베딩) 창고에 쌓아둔다. 이 창고가 벡터 DB다. 질문이 들어오면 질문도 같은 숫자로 바꾼다. 창고에서 제일 가까운 조각 몇 개를 꺼내 질문에 딸려 보내면 모델이 그걸 읽고 답한다.
...
[우아한형제들](https://techblog.woowahan.com/25900/)은 DB, 구글 드라이브, 구글 캘린더에 흩어진 교육 운영 정보를 한 번에 찾으려고 이 뼈대의 검색봇을 만들었고 이미 운영 중이던 Redis를 벡터 창고로 재활용했다. [SK하이닉스](https://aws.amazon.com/ko/blogs/tech/sk-hynix-rag-platfrom-analysis-evaluation/)도 OpenSearch를 벡터 DB로 쓰는 같은 골격으로 플랫폼을 구축했다.
...
실제 서비스에 넣어 숫자를 공개한 곳도 있다. [링크드인](https://arxiv.org/abs/2404.17723)은 고객지원 티켓을 트리 구조로 풀어 지식그래프로 엮었다. 약 6개월 운영에서 검색이 정답을 위에 올리는 지표(MRR)가 77.6% 좋아졌고 문제 해결 시간 중앙값이 28.6% 줄었다. [앤트그룹](https://arxiv.org/abs/2409.13731)은 그래프를 결합한 자체 프레임워크 KAG를 전자정부와 헬스케어 QA 실서비스에 적용했다고 밝혔다.
...
효과가 극적으로 갈리는 지점도 확인돼 있다. 그래프 DB를 파는 [FalkorDB](https://www.falkordb.com/blog/graphrag-accuracy-diffbot-falkordb/)의 블로그는 Diffbot 벤치마크를 인용해 이런 결과를 전한다. 질문에 등장하는 개체가 5개를 넘으면 그래프 없는 시스템의 정확도가 0%까지 떨어졌다. 그래프 쪽은 10개 이상에서도 버텼고 LLM 단독 16.7% 대 그래프 결합 56.2%였다. 파는 쪽 자료라는 건 감안하고 봐야 한다. 그래도 관계가 얽힌 질문일수록 그래프가 유리하다는 방향 자체는 다른 사례들과 어긋나지 않았다.
...
이 방향의 끝에 있는 게 요즘의 딥 리서치 제품들이다. [OpenAI](https://openai.com/index/introducing-deep-research/)의 Deep Research는 강화학습으로 모델이 다단계 리서치를 스스로 계획하고 실행하도록 만들었다. [앤트로픽](https://www.anthropic.com/engineering/multi-agent-research-system)의 Research 기능은 리드 에이전트가 계획을 세우고 서브 에이전트 여럿을 병렬로 부리는 구조다. 내부 평가에서 단일 에이전트보다 90.2% 나은 성능을 냈지만 토큰 소모가 일반 채팅의 약 15배였다. 단일 에이전트도 약 4배다.
...
[카사코리아](https://aws.amazon.com/ko/blogs/tech/kasakorea-agentic-cs-chatbot/)는 고객 문의 챗봇을 Bedrock의 Claude Sonnet, Titan 임베딩, LangGraph 워크플로, 전용 리랭커로 구성했다. 자주 묻는 질문과 유사도가 0.85를 넘으면 LLM을 부르지 않고 준비된 답을 즉시 반환하는 지름길이 있다.
...
싸고 빠르다는 게 최대 장점이다. 하이브리드 검색과 리랭커까지 붙여도 추가 비용이 크지 않은데 정확도는 눈에 띄게 오른다. 대신 검색 단계가 끼는 순간 응답은 느려진다. [SK하이닉스](https://aws.amazon.com/ko/blogs/tech/sk-hynix-rag-platfrom-analysis-evaluation/) 실험에서 첫 글자까지 걸리는 시간이 30~40% 길어졌다. 그래서 [카사코리아](https://aws.amazon.com/ko/blogs/tech/kasakorea-agentic-cs-chatbot/)는 자주 나오는 질문을 유사도 0.85 기준으로 걸러 LLM 없이 즉답하는 지름길을 팠다. 자료가 20만 토큰이 안 되면 검색 시스템 없이 통째로 넣으라는 [앤트로픽](https://www.anthropic.com/engineering/contextual-retrieval)의 조언도 같은 맥락이다. 작을 때는 단순한 쪽이 이긴다.

### 그래프 RAG

관계를 따라가는 능력을 얻는 대신 인덱싱 비용을 낸다. 개체와 관계 추출을 전부 LLM이 하다 보니 [GraphRAG 공식 저장소](https://github.com/microsoft/graphrag/discussions/440)에는 1,000페이지 PDF에 약 120달러가 들었다는 보고가 있고 벡터 임베딩이면 5달러도 안 될 자료가 50~200달러까지 뛴다는 [추정](https://www.paperclipped.de/en/blog/graph-rag-production/)도 있다. 이 돈이 아깝지 않은 조건은 하나다. 질문 대부분이 관계를 묻는 경우다. [링크드인](https://arxiv.org/abs/2404.17723)은 고객지원 티켓이 서로 얽혀 있는 자료라 그래프 비용을 내고도 해결 시간을 28.6% 줄이는 이득을 봤다. 반대로 단순 조회가 대부분인 서비스가 그래프부터 깔면 비싼 인덱스를 놀리는 셈이 된다.

### 에이전틱

여러 단계 작업을 처리하는 능력을 얻는 대신 토큰과 시간을 낸다. [앤트로픽](https://www.anthropic.com/engineering/multi-agent-research-system) 측정으로 멀티 에이전트는 일반 채팅의 약 15배를 태운다. 그래서 전부를 에이전트로 돌리는 회사는 없었다. [우버](https://www.uber.com/en-CA/blog/enhanced-agentic-rag/)는 답변 품질이 중요한 온콜 지원에만 에이전트 단계를 넣어 부정확한 조언을 60% 줄였고 [드롭박스](https://dropbox.tech/machine-learning/building-dash-rag-multi-step-ai-agents-business-users)는 단순 조회를 RAG에 남겨 질의 95%를 2초 안에 끝냈다. 비싼 경로는 비싼 값을 하는 질문에만 태운다는 게 공통 원칙이다.
